...
public class Reader {
    ...
    public static void read(Swagger swagger, Set<Class<?>> jobClassSet, String httpContext) {
        final Reader reader = new Reader(swagger);
        for (Class<?> entry : jobClassSet) {
            final ReaderContext context = new ReaderContext(swagger,
                    entry, entry, httpContext, null, false,
    ...
    private void read(ReaderContext context) {
        ...
        List<Method> allMethodList = Arrays.asList(context.getRefCls().getMethods());
        Collections.sort(allMethodList, new Comparator<Method>() {
            @Override
            public int compare(Method o1, Method o2) {
                return o1.getName().compareTo(o2.getName());
            }
        });
        Map<Method, Method> filterMethods = new LinkedHashMap<Method, Method>();
        ...
        for (Entry<Method, Method> entry : filterMethods.entrySet()) {
