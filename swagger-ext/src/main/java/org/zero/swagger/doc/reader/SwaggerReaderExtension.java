...
import org.zero.swagger.doc.model.JobParam;
...
public class SwaggerReaderExtension implements ReaderExtension {
    ...
    public void applyOperationId(Operation operation, Method method) {
        // 避免和其他类的方法同名
        operation.operationId(method.getName() + "......" + method.getDeclaringClass().getSimpleName());
    ...
    private String outputMethod(Method method) {
        try {
            ...
            sb.append("接口: ").append(method.getDeclaringClass().getInterfaces()[0].getSimpleName()).append(" ");
    ...
    public void applyTags(ReaderContext context, Operation operation, Method method) {
        ...
        Api apiAnnotation = context.getInterfaceCls().getAnnotation(Api.class);
        if (apiAnnotation == null) {
            apiAnnotation = context.getCls().getAnnotation(Api.class);
        }
        if (apiAnnotation != null) {
...
//		    bp.setIn("formData");
    ...
    public void applyParameters(ReaderContext context, Operation operation, Method method,
        ...
        try {
            ...
            if (method.getAnnotation(XxlJob.class) != null) {
                // 如果是xxl-job,默认生成requestBody参数
                parameterNames = new String[]{"JobParam"};
                genericParameterTypes = new Class[]{JobParam.class};
                parameterTypes = new Class[]{JobParam.class};
                parameterAnnotations = new Annotation[1][1];
                parameterAnnotations[0][0] = null;
                interfaceParamAnnotations = new Annotation[1][1];
                interfaceParamAnnotations[0][0] = null;
            }
